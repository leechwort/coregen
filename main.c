...
void generate_audio(coregen_cfg_t *cfg, float sample_length, float target_freq)
{
	// Create audio buffer
	int16_t buf[Fs * 20] = { 0 };
	int n;			// buffer index

	set_target_freq(cfg, target_freq);
...
int main() {
	...
	load_wt_from_file(&wt_one,
			"/home/artsin/Dev/coregen/waves/sharkfin_2048.cgw");

	load_wt_from_file(&wt_two,
			"/home/artsin/Dev/coregen/waves/tri_2048.cgw");
	allocate_wt(&wt_mix, WT_SIZE);

	morph_wt(&wt_one, &wt_two, &wt_mix, 0.35, MORPH_SUB);
